...
- автоматическая и ручная проверка обновлений Windows-клиента с загрузкой проверенного ZIP-пакета с сервера.
...
Клиент показывает установленную версию в боковом меню, автоматически проверяет новый релиз после входа и позволяет запустить ручную проверку кнопкой **Check updates**. Пакет загружается с рабочего сервера и принимается только после проверки размера и SHA-256.
