...
Актуально на 14 августа 2026 года. Версия клиента: **1.6.2**.
...
Готовый пакет появится в **artifacts\release\DieCutCatalog-1.6.2-win-x64.zip**. Для первой установки распакуйте архив, например, в **C:\Program Files\FLEXPRINT\DieCut Catalog** и запустите **DieCutCatalog.Desktop.exe**.

> Дальнейшие обновления клиент получает с сервера. Нажмите **Check updates**, подтвердите загрузку и установку. После проверки SHA-256 приложение закроется, обновится и запустится снова. База и PDF находятся на сервере и не затрагиваются.

## Первый вход

![Экран входа](images/login.png)

1. В поле **Адрес сервера** введите HTTPS-адрес API. Рабочий адрес FLEXPRINT: **https://diecutcatalog.duckdns.org**.
2. Введите электронную почту учётной записи и пароль.
3. Нажмите **Войти**. После успешного входа сессия сохраняется в зашифрованном Windows DPAPI-хранилище для текущего пользователя Windows.
4. При первом входе замените временный пароль. Повторный ввод данных не требуется до выхода из учётной записи или установки новой версии.

> Для удалённого сервера клиент принимает только HTTPS. HTTP разрешён только для **localhost**, **127.0.0.1** или явно включённого development-режима. При ошибке соединения откройте **https://diecutcatalog.duckdns.org/health** в браузере. Должен быть ответ **"Healthy"** без предупреждения о сертификате.
...
2. Поисковые выпадающие списки фильтруют каталог по валу, точным значениям L и B, материалу и фигуре. Фильтры применяются на сервере ко всему каталогу, а не только к текущей странице.
3. Нажатие заголовка столбца **L** или **B** сортирует весь результат по возрастанию и убыванию.
