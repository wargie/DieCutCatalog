...
Правый щелчок по значению открывает команды **Просмотреть**, **Редактировать**, **Удалить**, **Дублировать в текущий раздел**, **Копировать в…** и **Перенести в…**. Последние две команды позволяют выбрать целевой пользовательский справочник. Создание, изменение, CSV-импорт, архивирование, редактирование статьи, копирование, перенос и удаление фиксируются в общем журнале.

Архивный справочник работает только в режиме чтения. Сервер не разрешит добавить, импортировать, изменить или удалить значение, сохранить статью, скопировать позицию в архивный справочник либо перенести позицию из него. Чтобы изменить содержимое, сначала восстановите сам справочник. Простое скрытие кнопок в клиенте не используется как защита: правило проверяет API.
...
В общем журнале также отображаются административные изменения справочников. Для них система хранит инициатора, подтверждающего администратора для Danger Zone, объект операции и снимки данных до/после. Экспорт CSV содержит эти технические поля и correlation ID, по которому можно связать части одной операции.
...
Каждая генерация создаёт отдельный PDF и событие журнала. Старые версии не перезаписываются. Круг и квадрат можно сформировать только при одинаковых L и B. Для **специальной формы**, **перфорации** и любого неизвестного контура кнопка **Создать PDF** недоступна: используйте **Загрузить PDF** и прикрепите утверждённую схему с реальной геометрией. Сервер также отклонит такую автоматическую генерацию, поэтому ложный прямоугольный чертёж не будет сохранён. **Импорт PDF** создаёт новую карточку, но распознанные значения обязательно проверяются пользователем.
...
- Актуальный Windows-релиз: [DieCut Catalog 1.9.4](https://github.com/wargie/DieCutCatalog/releases/tag/v1.9.4)
- Скачать Windows-клиент: [DieCutCatalog-1.9.4-win-x64.zip](https://github.com/wargie/DieCutCatalog/releases/download/v1.9.4/DieCutCatalog-1.9.4-win-x64.zip)
